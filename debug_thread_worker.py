...
class ThreadPreProcessWorker(threading.Thread):
    def __init__(self, queue, batch_size, files, n_features, pos_file=None, neg_percentage=0.9, take_last_k_cycles=-1):
        ...
        self.data = pd.read_csv(self.files[self.permutation[0]], index_col=False)
        self.is_finished = False
        self.pos_df = pd.read_csv(pos_file, index_col=False) if pos_file else None
    ...
    def load_batch(self):
        ...
        # print('Time for batch preprocessing is: {}'.format(time.time() - start_time))
